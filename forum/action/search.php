...
if (isset($_REQUEST['submit']) && $_REQUEST['submit']) {
	...
	switch ($search_in) {
		case 'all':
			$sql_count = "SELECT COUNT(*) FROM " . prefix . "_forum_posts AS a INNER JOIN " . prefix . "_forum_topics AS b
									ON a.tid = b.id 
									WHERE MATCH (a.message, b.title) AGAINST ('{$string}' IN BOOLEAN MODE){$forums_id}";
			break;
		case 'post':
			$sql_count = "SELECT COUNT(*) FROM " . prefix . "_forum_posts a INNER JOIN " . prefix . "_forum_topics AS b
									ON a.tid = b.id 
									WHERE MATCH (a.message) AGAINST ('{$string}' IN BOOLEAN MODE){$forums_id}";
			break;
		case 'topic':
			$sql_count = "SELECT COUNT(*) FROM " . prefix . "_forum_topics AS b
	...
	switch ($search_in) {
		case 'all':
			$sql_two = 'SELECT a.`id`, a.`message`, a.`tid`, a.`author`, b.`id`, b.`title` 
			...
			break;
		case 'post':
			$sql_two = 'SELECT a.`id`, a.`message`, a.`tid`, a.`author`, b.`fid`, b.`title` 
									FROM `' . prefix . '_forum_posts` AS a INNER JOIN `' . prefix . '_forum_topics` AS b
									ON a.`tid` = b.`id` 
									WHERE MATCH (a.`message`) AGAINST (\'' . $string . '\' IN BOOLEAN MODE)' . $forums_id . '
			...
			break;
		case 'topic':
			$sql_two = 'SELECT `id`, `title`, `author`, `fid` 
