...
<div id="pm">
	<form name="form" method="POST" action="{{ php_self }}?action=delete">
		...
		<table class="content" border="0" cellspacing="0" cellpadding="0" align="center">
			<tr>
				<td width=100% colspan="6" class="contentHead">
					<img src="{{ admin_url }}/plugins/pm/img/nav.gif" hspace="8" align="left"/>{{ lang['pm:inbox'] }}
				</td>
			</tr>
			...
			<tr align="center">
				<td width="50%" colspan="2" class="contentHead"><a href="/plugin/pm/"><b>{{ lang['pm:inbox'] }}</b></a>
				</td>
				<td width="50%" colspan="2" class="contentHead">
					<a href="/plugin/pm/?action=outbox">{{ lang['pm:outbox'] }}</a></td>
				<td width="50%" colspan="2" class="contentHead">
					<a href="{{ php_self }}?action=set" align="right">{{ lang['pm:set'] }}</a></td>
			</tr>
			...
			<tr align="center">
				...
				<td width="5%" class="contentHead">
					<input class="check" type="checkbox" name="master_box" title="{{ lang['pm:checkall'] }}" onclick="javascript:check_uncheck_all(form)">
				</td>
			</tr>
			...
			{% for entry in entries %}
				<tr align="center">
					...
					<td class="contentEntry1">{% if (entry.viewed == 1) %}
							<img src="/engine/plugins/pm/img/viewed.yes.gif"/>{% else %}
							<img src="/engine/plugins/pm/img/viewed.no.gif"/>{% endif %}</td>
					<td class="contentEntry1">
						<input name="selected_pm[]" value="{{ entry.pmid }}" class="check" type="checkbox"/></td>
				</tr>
			{% endfor %}
	...
	<form name="pm" method="POST" action="{{ php_self }}?action=write">
		<input class="button" type="submit" value="{{ lang['pm:write'] }}"></form>
	</td>
	</tr>
	</table>
</div>