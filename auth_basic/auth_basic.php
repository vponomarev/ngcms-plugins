...
class auth_basic extends CoreAuthPlugin {
	...
	// $username	= ëîãèí
	// $password	= ïàðîëü
	// $auto_scan	= åñëè 1, òî ôóíêöèÿ ñàìà äîëæíà íàéòè íóæíûå ïàðàìåòðû ñðåäè POST'îâ
	...
	//
	// Ñîõðàíèòü â ÁÄ èíôîðìàöèþ î òîì, ÷òî ïîëüçîâàòåëü àâòîðèçîâàëñÿ
	// $dbrow	= ñòðîêà èç íàøåé òàáëèöû ïîëüçîâàòåëåé
	function save_auth($dbrow) {
		global $config, $mysql, $ip, $ngCookieDomain;

        	// ñîçäà¸ì random cookie
	...
	//
	// Ïðîâåðèòü àâòîðèçàöèþ ïîëüçîâàòåëÿ
	function check_auth() {
	...
	//
	// Âåðíóòü ìàññèâ ïàðàìåòðîâ, íåîáõîäèìûõ ïðè ðåãèñòðàöèè
	...
	//
	// Ïðîâåñòè ðåãèñòðàöèþ
	// params = ïàðàìåòðû ïîëó÷åííûå èç get_reg_params()
	// values = çíà÷åíèÿ äëÿ âûøåóêàçàííûõ ïàðàìåòðàõ
	// msg	= ñîîáùåíèå îá îøèáêå
	...
	function register(&$params, $values, &$msg) {
		...
		$userid = 0;
	 	...
	 	if (strlen($values['login'])<3) {
	 		// Ñëèøêîì êîðîòêèé ëîãèí
	 		$msg = $lang['auth_login_short'];
	 		return 0;
	 	}

		// Ïðîâåðÿåì ëîãèí íà çàïðåùåííûå ñèìâîëû
		$csError = false;
		switch (pluginGetVariable('auth_basic', 'regcharset')) {
			case 0:
				...
				break;
			case 1:
				if (!preg_match('#^[À-ßà-ÿ¸¨0-9\.\_\-]+$#s', $values['login'])) {
					$csError = true;
				}
				break;
			case 2:
				if (!preg_match('#^[À-ßà-ÿ¸¨A-Za-z0-9\.\_\-]+$#s', $values['login'])) {
	 	...
	 	if ($config['register_type'] >= 3) {
	 		if (strlen($values['password']) < 3) {
		 		// Ñëèøêîì êîðîòêèé ïàðîëü
		 		$msg = $lang['auth_pass_short'];
		 		return 0;

		 	} else if ($values['password'] != $values['password2']) {
		 		// Íåñîâïàäåíèå ïàðîëåé
		...
		// Ãðóïïà ïîëüçîâàòåëÿ ïî óìîë÷àíèþ
		...
		// Îïðåäåëÿåì äåéñòâèÿ â çàâèñèìîñòè îò òèïà ðåãèñòðàöèè
		switch ($config['register_type']) {

			// 0 - Ìãíîâåííàÿ [àâòîãåíåðàöèÿ ïàðîëÿ, áåç email íîòèôèêàöèè]
			case 0:
			...
			// 1 - Ïðîñòàÿ [àâòîãåíåðàöèÿ ïàðîëÿ, ñ email íîòèôèêàöèåé]
			case 1:
				$newpassword = MakeRandomPassword();
				$mysql->query("INSERT INTO ".uprefix."_users (name, pass, mail, status, reg, last) VALUES (".db_squote($values['login']).", ".db_squote(EncodePassword($newpassword)).", ".db_squote($values['email']).", ".$regGroup.", '".$add_time."', '')");
				$userid			=	$mysql->result('select LAST_INSERT_ID()');
			...
			// 2 - Ñ ïîäòâåðæäåíèåì [àâòîãåíåðàöèÿ ïàðîëÿ, ïàðîëü îòïðàâëÿåòñÿ íà email àäðåñ è íå ïîêàçûâàåòñÿ â àäìèíêå]
			case 2:
				$newpassword	=	MakeRandomPassword();
				$mysql->query("INSERT INTO ".uprefix."_users (name, pass, mail, status, reg, last) VALUES (".db_squote($values['login']).", ".db_squote(EncodePassword($newpassword)).", ".db_squote($values['email']).", ".$regGroup.", '".$add_time."', '')");
				$userid			=	$mysql->result('select LAST_INSERT_ID()');
			...
			// 3 - Ðó÷íàÿ ñ íîòèôèêàöèåé [ðó÷íàÿ ãåíåðàöèÿ ïàðîëÿ, email íîòèôèêàöèÿ]
			case 3:
			...
			// 4 - Ðó÷íàÿ ñ ïîäòâåðæäåíèåì [ðó÷íàÿ ãåíåðàöèÿ ïàðîëÿ, ïîäòâåðæäåíèå email àäðåñà]
			case 4:
	...
	//
	// Âåðíóòü ìàññèâ ïàðàìåòðîâ, íåîáõîäèìûõ äëÿ âîññòàíîâëåíèÿ ïàðîëÿ
		...
		if (isset($params['login'])) {
			...
			switch (pluginGetVariable('auth_basic', 'regcharset')) {
				case 0:
					...
					break;
				case 1:
					if (!preg_match('#^[À-ßà-ÿ¸¨0-9\.\_\-]+$#s', $params['login'])) {
						$csError = true;
					}
					break;
				case 2:
	...
	//
	// Ïîäòâåðæäåíèå âîññòàíîâëåíèÿ ïàðîëÿ
	//
