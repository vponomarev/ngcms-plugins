...
if (!(filter_var($email, FILTER_VALIDATE_EMAIL)) || empty($email)) { msg(array("type" => "error", "text" => $lang['subscribe_comments:err.wrongemail'])); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return; }
...
//var_dump($_REQUEST);
	...
	switch ($unsubscribe)
	{
		...
		{
			if(empty($altname)) { msg(array("type" => "error", "text" => $lang['subscribe_comments:err.wrongparams'])); return; }
			$row = $mysql->record("select * from ".prefix."_news where alt_name='".$altname."'");
			if(empty($row)) { msg(array("type" => "error", "text" => $lang['subscribe_comments:err.wrongaltname'])); return; }
			...
			msg(array("type" => "message", "text" => str_replace(array('{title}'), array($row['title']), $lang['subscribe_comments:msg.unsubscribepageok']))); return;
			}
			else
			{
			msg(array("type" => "error", "text" => $lang['subscribe_comments:err.unsubscribepageyet'])); return;
			}
		}
		else {
			msg(array("type" => "error", "text" => $lang['subscribe_comments:err.wrongparams'])); return;
			 }
		break;
		...
		if (!empty($email))
		{
			...
			msg(array("type" => "message", "text" => $lang['subscribe_comments:msg.unsubscribeall'])); return;
			}
			else
			{
			msg(array("type" => "error", "text" => $lang['subscribe_comments:err.unsubscribeall'])); return;
			}
		...
		default:
		if(empty($altname)) { msg(array("type" => "error", "text" => $lang['subscribe_comments:err.wrongparams'])); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return; }
		
		$row = $mysql->record("select * from ".prefix."_news where alt_name='".$altname."'");
		if(empty($row)) { msg(array("type" => "error", "text" => $lang['subscribe_comments:err.wrongaltname'])); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return; }
		
		if(!($nrow = $mysql->record("select * from ".prefix."_subscribe_comments where news_id='".$row['id']."' and news_altname='".$row['alt_name']."' and user_email='".$email."'")))
		{
		$mysql->query("insert into ".prefix."_subscribe_comments (user_email, news_id, news_altname) values ('".$email."', '".$row['id']."', '".$row['alt_name']."')");
		msg(array("type" => "message", "text" => $lang['subscribe_comments:msg.subscribepageok'])); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return;
		}
		else
		{
		msg(array("type" => "message", "text" => $lang['subscribe_comments:err.subscribepageyet'])); header('Refresh: '.$timeout.'; URL='.secure_html($_SERVER["HTTP_REFERER"]).''); return;
		}
		break;
	}
/*
