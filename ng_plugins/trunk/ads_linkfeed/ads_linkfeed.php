...
if (!defined('LINKFEED_USER')){
	define('LINKFEED_USER', extra_get_param('ads_linkfeed', 'linkfeed_user'));
...
function plugin_ads_linkfeed() {
	...
	$count = abs(intval(extra_get_param('ads_linkfeed', 'bcount')));
	if (!$count) $count = 1;

	$blen = array();
	foreach (split(",", extra_get_param('ads_linkfeed', 'blength')) as $br) {
		$blen []= intval(trim($br));
	}
	...
	$linkfeed = new LinkfeedClient(array(
		'multi_site' => extra_get_param('ads_linkfeed', 'multisite')?true:false,
