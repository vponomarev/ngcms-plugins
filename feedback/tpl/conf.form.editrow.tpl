...
<form action="?mod=extra-config&plugin=feedback&action=editrow&form_id={{ formID }}&name={{ fieldName }}" method="post" name="feedbackform">
	...
	<table border="0" cellspacing="1" cellpadding="1" class="content">
		...
		<tr class="contRow1">
			<td width="50%">Автозаполнение:<br/>
				<small><b>Нет</b> - пользователь должен самостоятельно заполнять поле<br/><b>параметром</b> - значение
					поля может передаваться в качестве параметра к URL'у вызова<br/><b>связкой с новостью</b> - значение
					поля может браться из одноимённого поля плагина <b>xfields</b> связанной новости<br/><b>связка с
						профилем</b> - значение поля может браться из одноимённого поля плагина <b>xfields</b> профиля
					пользователя
				</small>
			</td>
			<td>
		...
		<tr class="contRow1">
			<td width="50%">Блокировка:<br/>
				<small><b>Нет</b> - поле доступно для редактирования<br/><b>Да</b> - пользователь не может редактировать
					данное поле<br/><b>Да, невидимо</b> - поле невидимо, его редактирование запрещено
				</small>
			</td>
			<td>
				<select name="block">
